...
function removeAzureADUserFromAllOffice365Groups
{
    ...
    $AADUser = "zz-o365admin@maehren-gruppe.de"
    $AADPasswordFile = "\\maehren-gruppe.de\daten\Admins\Scripte\Onboarding\Keys\AzureADPwd.txt"
    $AADKeyFile = "\\maehren-gruppe.de\daten\Admins\Scripte\Onboarding\Keys\AzureADKey.key"
        ...
        foreach ($user in $users)
        {
            ...
            Try
            {
                ...
                if ($AzureADGroups -ne $null)
                {
                    ...
                    {
                        if(($AzureADGroup.DirSyncEnabled -eq $null) -and ($AzureADGroup.DisplayName -ne "Alle Benutzer"))
                        {
